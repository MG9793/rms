        ...
        <div class="card px-4">
            <div class="card-body">                          
                <div class="headReport">
                    <h5 class="fw-bold p-2 text-center">กรุณาเลือกข้อมูล | รายงานภาษีซื้อประจำเดือน <?php echo $_SESSION['thisMonth']; ?></h5>
                </div>
                ...
                <?php
                    ...
                    $stmt = $conn->query("SELECT * FROM bill_head WHERE DATE_FORMAT(buy_date, '%Y-%m') = '$thisMonth' AND report = 'N' ORDER BY buy_date ASC");
                    // $stmt->execute();
                    $tax = $stmt->fetchAll();

                    // query รวมยอดซื้อ
                    $sumBuy = $conn->query("SELECT SUM(sum) AS sumbuy FROM bill_head WHERE DATE_FORMAT(buy_date, '%Y-%m') = '$thisMonth' AND report = 'N'");
                    // $sumBuy->execute();
                    $query_sumBuy = $sumBuy->fetch(PDO::FETCH_ASSOC);

                    // query รวมยอด VAT
                    $sumVAT = $conn->query("SELECT SUM(vat) AS sumvat FROM bill_head WHERE DATE_FORMAT(buy_date, '%Y-%m') = '$thisMonth' AND report = 'N'");
                    // $sumVAT->execute();
                    $query_sumVAT = $sumVAT->fetch(PDO::FETCH_ASSOC);

                    // query รวมยอดซื้อที่ออกรายงานแล้ว
                    $sumBuy_reported = $conn->query("SELECT SUM(sum) AS sumbuy_reported FROM bill_head WHERE DATE_FORMAT(buy_date, '%Y-%m') = '$thisMonth' AND report = 'Y'");
                    // $sumBuy_reported->execute();
                    $query_sumBuy_reported = $sumBuy_reported->fetch(PDO::FETCH_ASSOC);

                    // query ยอด VAT ที่ออกรายงานแล้ว
                    $sumVAT_reported = $conn->query("SELECT SUM(vat) AS sumvat_reported FROM bill_head WHERE DATE_FORMAT(buy_date, '%Y-%m') = '$thisMonth' AND report = 'Y'");
                    // $sumVAT_reported->execute();
                ...
                <form action="../db/db_filterTaxSummary.php" method="POST">
                    <div class="row mt-3">
                        ...
                        <div class="col-md-2 text-center">
                            <label for="selectedVAT" class="form-label fw-bold">รวมยอด VAT ที่เลือก :</label>

        ...
        <div class="card px-4">
            <div class="card-body">                          
                <div class="headReport">
                    <h5 class="fw-bold p-2 text-center">รายงานภาษีซื้อที่ออกรายงานแล้ว</h5>
                </div>
                ...
                <?php
                    ...
                    $stmt = $conn->query("SELECT * FROM bill_head WHERE report = 'Y' ORDER BY buy_date ASC");
                    // $stmt->execute();
                    $tax = $stmt->fetchAll();

                    // query รวมยอดซื้อทั้งหมด ที่ยังไม่ออกรายงาน
                    $sumBuy = $conn->query("SELECT SUM(sum) AS sumbuy FROM bill_head WHERE report = 'N'");
                    // $sumBuy->execute();
                    $query_sumBuy = $sumBuy->fetch(PDO::FETCH_ASSOC);

                    // query รวมยอด VAT ทั้งหมด ที่ยังไม่ออกรายงาน
                    $sumVAT = $conn->query("SELECT SUM(vat) AS sumvat FROM bill_head WHERE report = 'N'");
                    // $sumVAT->execute();
                    $query_sumVAT = $sumVAT->fetch(PDO::FETCH_ASSOC);

                    // query รวมยอดซื้อที่ออกรายงานแล้ว
                    $sumBuy_reported = $conn->query("SELECT SUM(sum) AS sumbuy_reported FROM bill_head WHERE report = 'Y'");
                    // $sumBuy_reported->execute();
                    $query_sumBuy_reported = $sumBuy_reported->fetch(PDO::FETCH_ASSOC);

                    // query ยอด VAT ที่ออกรายงานแล้ว
                    $sumVAT_reported = $conn->query("SELECT SUM(vat) AS sumvat_reported FROM bill_head WHERE report = 'Y'");
